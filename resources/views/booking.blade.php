                ...
                <div class="flex items-start w-full gap-1 sm:gap-2 lg:gap-4">
                    ...
                    <div class="flex items-start flex-1 min-w-0" id="step1-indicator">
                        <div class="flex flex-col items-center flex-1 min-w-0">
                            <div class="w-10 h-10 lg:w-12 lg:h-12 rounded-full bg-pink-500 text-white flex items-center justify-center font-bold text-sm lg:text-base border-4 border-white shadow-lg step-circle active flex-shrink-0">
                                1
                            </div>
                    ...
                    <div class="flex items-start flex-1 min-w-0" id="step2-indicator">
                        <div class="flex flex-col items-center flex-1 min-w-0">
                            ...
                            <span class="mt-2 text-[10px] sm:text-xs lg:text-sm font-medium text-gray-500 text-center whitespace-nowrap leading-tight">{{ __('booking.select_tour_type') }}</span>
                        </div>
                        <div class="flex-1 h-1 bg-gray-200 mx-1 sm:mx-2 lg:mx-4 step-line mt-5 lg:mt-6 flex-shrink"></div>
                    </div>
            ...
            <div class="bg-white overflow-visible">
                    ...
                    <div class="max-w-2xl mx-auto">
                        ...
                        <div class="relative z-50">
                            <button type="button" id="tourSelectBtn" 
                                class="w-full px-4 py-3 text-left border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-500 focus:border-pink-500 outline-none bg-white flex items-center justify-between cursor-pointer hover:border-pink-400 transition-colors">
                            ...
                            <div id="tourSelectDropdown" class="hidden absolute z-[60] w-full mt-1 bg-white border-2 border-gray-300 rounded-lg shadow-xl overflow-hidden max-h-96 overflow-y-auto">
                                <div class="py-1" id="tourSelectOptions">
                                    @foreach($tours as $tour)
                                        <button type="button" class="w-full px-4 py-3 text-left text-sm text-gray-700 hover:bg-pink-50 hover:text-pink-600 transition-colors flex items-start gap-3 tour-option" 
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <div id="tourTypeDropdown" class="hidden absolute z-50 w-full mt-1 bg-white border-2 border-gray-300 rounded-lg shadow-xl overflow-hidden">
                                ...
                                <div id="selectedTourTypeDisplay" class="hidden mt-3 p-4 bg-pink-50 rounded-lg border border-pink-200">
                            ...
                            <div>
                                ...
                                <input type="text" id="tourStartDate" 
                                    class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-500 focus:border-pink-500 outline-none cursor-pointer"
                            ...
                            <div>
                                ...
                                <div>
                                    ...
                                    <input type="number" id="adultsCount" min="1" value="2" 
                                        class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-500 focus:border-pink-500 outline-none">
                                </div>
                            </div>
                        </div>
                ...
                <div id="step3" class="step-content hidden p-6 lg:p-8">
                    ...
                    <div class="space-y-8">
                        <!-- Bus Service Option -->
                        <div>
                            <div class="flex items-center gap-3 p-4 border-2 border-gray-300 rounded-lg hover:border-pink-400 transition-colors">
                            ...
                            <div id="busServiceSection" class="hidden mt-6 space-y-6">
                                ...
                                <div>
                                    ...
                                    <input type="text" id="busDepartureDate" 
                                        class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-500 focus:border-pink-500 outline-none cursor-pointer mb-3"
                                        placeholder="{{ __('tours.select_date') }}"
                                        readonly>
                        ...
                        <div>
                            ...
                            <div id="giftOptions" class="flex gap-4 overflow-x-auto pb-2 scrollbar-thin scrollbar-thumb-pink-300 scrollbar-track-gray-100">
...
<script>
    ...
    function nextStep(step) {
        ...
        if (step === 3 && !bookingData.tourTypeId) {
            alert('{{ __('booking.select_tour_type_first') }}');
            return;
        }
    ...
    // Tour Selection
    function selectTour(tourId, tourSlug, tourName, element) {
        // Update button text
    ...
    async function loadTourTypes() {
        ...
        try {
            const response = await fetch(`{{ route('api.tours.types', ':id') }}`.replace(':id', selectedTourId));
            const types = await response.json();
            
            const container = document.getElementById('tourTypeOptions');
            container.innerHTML = '';
            
            if (types.length === 0) {
                container.innerHTML = `<div class="px-4 py-2 text-sm text-gray-500 text-center">${translations.no_tours_available}</div>`;
                return;
            }
            ...
            types.forEach(type => {
                ...
                option.type = 'button';
                option.className = 'w-full px-4 py-3 text-left text-sm text-gray-700 hover:bg-pink-50 hover:text-pink-600 transition-colors flex items-center justify-between';
                option.onclick = () => selectTourType(type.id, type.name, type.price, option);
                option.innerHTML = `
                    <span>${type.name}</span>
                    <span class="font-semibold text-pink-600">${formatPrice(type.price)} VND</span>
                `;
    ...
    function selectTourType(typeId, typeName, typePrice, element) {
        document.getElementById('tourTypeText').textContent = typeName;
        document.getElementById('selectedTourTypeId').value = typeId;
        document.getElementById('tourTypeDropdown').classList.add('hidden');
        
        // Ensure typePrice is a number
        const price = parseFloat(typePrice) || 0;
        bookingData.tourPrice = price;
        bookingData.tourTypeId = typeId;
        bookingData.tourTypeName = typeName;
        
        console.log('Tour type selected:', { typeId, typeName, typePrice, price, bookingData });
        
        document.getElementById('tourPrice').textContent = formatPrice(price) + ' VND';
        document.getElementById('selectedTourTypeDisplay').classList.remove('hidden');
        
        // Remove selected class from all options
        document.querySelectorAll('#tourTypeOptions button').forEach(btn => {
    ...
    function checkStep2Complete() {
        const hasTourType = bookingData.tourTypeId !== null;
        const hasDate = document.getElementById('tourStartDate').dataset.value !== undefined;
        document.getElementById('step2-next').disabled = !(hasTourType && hasDate);
    }
    ...
    async function loadGifts() {
        try {
            ...
            gifts.forEach(gift => {
                ...
                giftOption.className = 'border-2 border-gray-300 rounded-lg p-4 cursor-pointer hover:border-pink-500 transition-colors flex-shrink-0 w-32';
    ...
    async function loadBusStartingPoints() {
        try {
            ...
            points.forEach(point => {
                const option = document.createElement('button');
                option.type = 'button';
                option.className = 'w-full px-4 py-2 text-left text-sm text-gray-700 hover:bg-pink-50 hover:text-pink-600 transition-colors';
    ...
    async function loadBusServices(direction, location) {
        try {
            ...
            services.forEach(service => {
                ...
                serviceDiv.className = 'border-2 border-gray-300 rounded-lg p-4 cursor-pointer hover:border-pink-500 transition-colors';
                ...
                serviceDiv.innerHTML = `
                    <div class="flex items-start gap-3">
                        <input type="radio" name="bus_${direction}" value="${service.id}" 
                            class="mt-1 text-pink-500 focus:ring-pink-500">
                        <div class="flex-1">
                            <h4 class="font-semibold text-gray-900">${service.name}</h4>
                            <p class="text-sm text-gray-600">${service.departure_time || ''}${service.arrival_time ? ' - ' + service.arrival_time : ''}</p>
                            <p class="text-sm font-semibold text-pink-600 mt-1">${formatPrice(service.price)} VND</p>
                        </div>
                    </div>
                `;
    ...
    function updateBookingSummary() {
        ...
        const tourTypeLabel = translations.select_tour_type;
        ...
        let html = `
            ...
            <div class="flex justify-between py-2 border-b border-gray-200">
                <span class="text-gray-600">${tourTypeLabel}:</span>
                <span class="font-semibold text-gray-900">${bookingData.tourTypeName || translations.not_available}</span>
            </div>
            <div class="flex justify-between py-2 border-b border-gray-200">
    ...
    function proceedToCheckout() {
        ...
        if (bookingData.tourTypeId) {
            params.append('tour_type_id', bookingData.tourTypeId);
        }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (tourTypeBtn && tourTypeDropdown) {
            tourTypeBtn.addEventListener('click', (e) => {
                e.stopPropagation();
                tourTypeDropdown.classList.toggle('hidden');
            });
            document.addEventListener('click', (e) => {
