...
@section('title', __('checkout.title') . ' - Alley Homestay - Ha Giang Loop')
                    ...
                    <div class="lg:col-span-2 w-full min-w-0">
                        <div class="bg-white rounded-lg shadow-lg border border-gray-200 p-4 sm:p-6 mb-6 w-full">
                            ...
                            <div class="border-b border-gray-200 pb-4 mb-4">
                                ...
                                <div class="text-sm text-gray-600 space-y-1">
                                    <p><strong>{{ __('checkout.start_date') }}:</strong>
                                        {{ $tourStartDate ? \Carbon\Carbon::parse($tourStartDate)->format('d/m/Y') : __('checkout.not_selected') }}
                                    </p>
                                ...
                                <div class="border-b border-gray-200 pb-4 mb-4">
                                    ...
                                    @if($outboundBus)
                                        <div class="text-sm text-gray-600 mb-2">
                                            <p><strong>{{ __('checkout.outbound') }}:</strong> {{ $outboundBus->name }} -
                                                {{ $outboundBus->departure_time }}</p>
                                            <p class="text-pink-600 font-medium">
                                                {{ number_format($outboundBus->price, 0, ',', '.') }} VND</p>
                                        </div>
                                    @endif
                                    @if($returnBus)
                                        <div class="text-sm text-gray-600">
                                            <p><strong>{{ __('checkout.return') }}:</strong> {{ $returnBus->name }} -
                                                {{ $returnBus->departure_time }}</p>
                                            <p class="text-pink-600 font-medium">{{ number_format($returnBus->price, 0, ',', '.') }}
                                                VND</p>
                                        </div>
                                    @endif
                                </div>
                            ...
                            @if($gift)
                                <div class="border-b border-gray-200 pb-4 mb-4">
                                    ...
                                    <div class="flex items-center gap-3">
                                        @if($gift->image)
                                            <img src="{{ Storage::url($gift->image) }}" alt="{{ $gift->name }}"
                                    ...
                                    <div class="text-sm text-gray-600">
                                        ...
                                        <p class="text-xs text-gray-500">
                                            ...
                                            @if($accommodation->price_per_night > 0)
                                                ...
                                                x {{ $tour->nights }} night(s)
                                                = {{ number_format($accommodation->price_per_night * $tour->nights, 0, ',', '.') }}₫
                                            @else
                                                - (No fees)
                                            @endif
                                        </p>
                            ...
                            <div class="pt-4">
                                <div class="flex justify-between items-center">
                                    ...
                                    <span
                                        class="text-2xl font-bold text-pink-600">{{ number_format($totalPrice, 0, ',', '.') }}
                                        VND</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-white rounded-lg shadow-lg border border-gray-200 p-4 sm:p-6 w-full">
                            ...
                            <form action="{{ route('checkout.store') }}" method="POST" class="w-full" id="checkoutForm">
                                ...
                                <input type="hidden" name="outbound_bus_service_id"
                                    value="{{ $outboundBus ? $outboundBus->id : '' }}">
                                <input type="hidden" name="return_bus_service_id"
                                    value="{{ $returnBus ? $returnBus->id : '' }}">
                                ...
                                @if($adults > 1)
                                    ...
                                    <div class="mb-6 w-full">
                                        <h3 class="text-lg font-semibold text-gray-900 mb-4">
                                            {{ __('checkout.additional_passengers') }}</h3>
                                        <p class="text-sm text-gray-600 mb-4">
                                            {{ __('checkout.additional_passengers_description', ['count' => $adults - 1]) }}</p>

                                        <div id="additionalPassengersContainer" class="space-y-4">
                                            @for($i = 2; $i <= $adults; $i++)
                                                <div class="border border-gray-200 rounded-lg p-4 bg-gray-50">
                                                    <h4 class="text-md font-medium text-gray-700 mb-3">
                                                        {{ __('checkout.passenger') }} {{ $i }}</h4>
                                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                        <div>
                                                            <label for="passenger_name_{{ $i }}"
                                                                class="block text-sm font-medium text-gray-700 mb-2">
                                                                {{ __('checkout.full_name') }} <span class="text-red-500">*</span>
                                                            </label>
                                                            <input type="text" name="additional_passengers[{{ $i - 2 }}][name]"
                                                                ...
                                                                value="{{ old('additional_passengers.' . ($i - 2) . '.name') }}"
                                                                required>
                                                        ...
                                                        <div>
                                                            <label for="passenger_country_{{ $i }}"
                                                                class="block text-sm font-medium text-gray-700 mb-2">
                                                                {{ __('checkout.country') }} <span class="text-red-500">*</span>
                                                            </label>
                                                            <input type="text" name="additional_passengers[{{ $i - 2 }}][country]"
                                                                id="passenger_country_{{ $i }}"
                                                                ...
                                                                value="{{ old('additional_passengers.' . ($i - 2) . '.country') }}"
                                                                required>
                        ...
                        <div class="bg-white rounded-lg shadow-lg border border-gray-200 p-4 sm:p-6 sticky top-4 w-full">
                            ...
                            <div class="space-y-3 text-sm">
                                ...
                                <div class="flex justify-between gap-2">
                                    ...
                                    <span
                                        class="font-medium text-gray-900 text-right">{{ $tourStartDate ? \Carbon\Carbon::parse($tourStartDate)->format('d/m/Y') : __('checkout.not_selected') }}</span>
                                </div>
                                <div class="flex justify-between gap-2">
                                    <span class="text-gray-600 flex-shrink-0">{{ __('checkout.people') }}:</span>
                                    <span class="font-medium text-gray-900 text-right">{{ $adults }}
                                        {{ __('checkout.adult_s') }}{{ $children > 0 ? ', ' . $children . ' ' . __('checkout.child_ren') : '' }}</span>
                                </div>
                                @if($useBusService)
                                ...
                                <div class="pt-3 border-t border-gray-200">
                                    <div class="flex justify-between items-center">
                                        <span class="text-lg font-bold text-gray-900">{{ __('common.total') }}:</span>
                                        <span
                                            class="text-xl font-bold text-pink-600">{{ number_format($totalPrice, 0, ',', '.') }}
                                            VND</span>
                                    </div>
                                </div>
                            </div>
